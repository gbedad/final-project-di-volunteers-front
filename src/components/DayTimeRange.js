...
const handleRemoveDayTimeRange = (index) => {
  ...
  const handleSaveDayTimeRanges = async () => {
    ...
    try {
      ...
      const response =  await axios.post(`${process.env.REACT_APP_BASE_URL}/create-skill/${userLogged.user.id}`, {when_day_slot : JSON.stringify(dayTimeRanges)}, {
    ...
    <div>
      ...
      {dayTimeRanges.map((dayTimeRange, index) => (
        <Grid container spacing={2} key={index} style={{ marginTop: "16px" }}>
          ...
          <Grid item xs={1}>
            <Button
              onClick={() => handleRemoveDayTimeRange(index)}
            >

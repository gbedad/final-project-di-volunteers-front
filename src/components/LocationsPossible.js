 ...
 useEffect(() => {
  ...
  const handleSaveLocationsPossible = async () => {
    try {
      ...
      const response =  await axios.post(`${process.env.REACT_APP_BASE_URL}/create-skill/${userLogged.user.id}`, {where_location : locationsPossible}, {
      ...
      {locationsPossible.map((locationPossible, index) => (
        <Grid container spacing={2} key={index} style={{ marginTop: "16px" }}>
          <Grid item xs={8}>
            ...
            >
              <MenuItem value="Maison des Associations">Maison des Associations</MenuItem>
